...
class CircRNADiffusionModel(nn.Module):
    ...
    def __init__(self, config: Optional[CircDiffusionConfig] = None):
        ...
        # TrunkOutput protocol + immune-fingerprint head hook (scheme choice (c)).
        # Attach externally via attach_immune_head(head).
        self.trunk_adapter = TrunkOutputAdapter(
            d_model=self.config.d_node,  # S4's d_model is d_node
            c_z=self.config.d_node,      # No d_pair, use d_node
            structure_method="circrna_ddpm_egnn_guided",
        )
    ...
    def attach_immune_head(self, head: nn.Module) -> None:
        ...
        ImmuneFingerprintHeads). Called at the end of _sample with
        (sequence_repr, pair_repr, torus_coords=None, pair_probs)."""
    ...
    def _encode_single_for_trunk(
        ...
        """Run graph build + EGNN layers ONCE on the final sampled coords to
        produce the per-residue node representation (single_repr) for TrunkOutput.

        EGNN is coords-equivariant, so we feed the actual final coords (not
        zeros) — the node features then encode the geometric state of the
        predicted structure, which is exactly what the synthesizer + immune
        head want. t_emb is zeroed (we only want the structure-conditioned
        trunk state, not a diffusion-step signal).
        ...
        cond = self.condition_encoder(
            seq_tokens, None, temperature, pH, Mg_conc, Na_conc,
        )
        t_emb = torch.zeros(B, self.config.d_node, device=device)
        ...
        for layer in self.egnn_layers:
            node_feat, coords_i = layer(node_feat, coords_i, edge_index, edge_feat)

        return node_feat  # (B, L, d_node)
    ...
    def _sample(self, seq_tokens, pair_probs, ss_tokens,
        ...
        single_for_trunk = self._encode_single_for_trunk(
            seq_tokens, coords, pair_probs,
            temperature, pH, Mg_conc, Na_conc,
        )
        trunk_out = {
            "coords": coords,
        ...
        # S4 outputs Cartesian coords → pass them as cartesian_coords so the
        # DRACH m6A head uses the 3D kNN-density exposure proxy (path 2).
        if self.immune_head is not None:
            adapted["immune"] = self.immune_head(
                ...
                torus_coords=None,  # S4 无 torus 参数
                pair_probs=pair_probs,
                cartesian_coords=coords,  # (B, L, 3) 真实 3D 结构
            )
