...
class CircMambaDiffusionModel(nn.Module):
    ...
    def __init__(self, config: Optional[CircMambaConfig] = None):
        ...
        self.trunk_adapter = TrunkOutputAdapter(
            d_model=self.config.d_model,
            c_z=self.config.d_model,  # S7 doesn't have d_pair, use d_model
            structure_method="circrna_mamba_diffusion",
        )
    ...
    def attach_immune_head(self, head: nn.Module) -> None:
        ...
        ImmuneFingerprintHeads). The head is called at the end of _sample
    ...
    def _encode_single_for_trunk(
        ...
        """Run condition encoder + BiMamba + local attn ONCE at the end of
        _sample to produce the per-residue representation (single_repr) for
        TrunkOutput.

        Cheap single forward (~10% extra time) without re-running the full
        diffusion loop. x is zeroed (coords feature absent here) so this is
        purely the sequence-conditioned trunk state — that's all the
        synthesizer needs to build a learned pair_repr.
        """
        B, L = seq_tokens.shape
        device = seq_tokens.device

        cond = self.condition_encoder(seq_tokens, temperature, pH, Mg_conc, Na_conc)
        h = cond  # x = 0, t_emb = 0 (we only want the sequence trunk state)
    ...
    def _sample(self, seq_tokens, pair_probs, ss_tokens,
        ...
        single_for_trunk = self._encode_single_for_trunk(
            seq_tokens, temperature, pH, Mg_conc, Na_conc,
        )
        trunk_out = {
            "coords": coords,
        ...
        # S7's diffusion loop outputs Cartesian coords → pass as
        ...
        if self.immune_head is not None:
            adapted["immune"] = self.immune_head(
                ...
                torus_coords=None,  # S7 无 torus 参数
