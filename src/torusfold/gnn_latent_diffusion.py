...
class GNNLatentDiffusionModel(nn.Module):
    ...
    def __init__(self, config: Optional[GNNLatentConfig] = None):
        ...
        # S6 operates in a latent diffusion space; it has no explicit pair_probs
        # input and no torus parameterization. The synthesizer builds a learned
        # pair_repr from the per-residue latent + a uniform pair prior.
        self.trunk_adapter = TrunkOutputAdapter(
            d_model=self.config.d_latent,
            c_z=self.config.d_latent,
            structure_method="gnn_latent_diffusion",
        )
        self.immune_head: Optional[nn.Module] = None

    def attach_immune_head(self, head: nn.Module) -> None:
        """Attach a pluggable immune-fingerprint head. Called at the end of
        forward with (sequence_repr, pair_repr, torus_coords=None, pair_probs=None)."""
    ...
    def forward(
        ...
        # S6 operates in latent diffusion: encoder → latent → decoder.
        # sequence_repr = latent (B, L, d_latent), pair_probs = None,
        # pair_repr 由 synthesizer 从 latent 合成（uniform fallback）。
        trunk_out = {
            "coords": coords,
            "sequence_repr": latent,
            "pair_probs": None,  # S6 有 no explicit pair_probs input
            "structure_method": "gnn_latent_diffusion",
        }
        adapted = self.trunk_adapter(trunk_out)

        # Optional immune-fingerprint head.
        # S6's decoder outputs Cartesian coords → pass as cartesian_coords so
        # the DRACH m6A head + PKR sasa head use the 3D kNN-density exposure
        # proxy (path 2), not the 2D pair_probs fallback.
        if self.immune_head is not None:
            adapted["immune"] = self.immune_head(
                ...
                torus_coords=None,  # S6 无 torus 参数
                pair_probs=None,    # S6 无显式 pair_probs input
                cartesian_coords=coords,  # (B, L, 3) decoder 输出的真实 3D 结构
            )
