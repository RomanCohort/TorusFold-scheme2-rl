...
class DilatedRingAttention(nn.Module):
    """Dilated multi-rate attention on the circRNA ring (BSJ-aware).

    Position-prior counterpart to ViennaRNA's chemistry-prior sparse mask.
    Each head h attends to keys at positions (i + k·r_h) % L for all k,
    so coverage scales as O(L / r_h) per head while staying a fixed pattern.
    Different heads use different dilation rates (e.g. {2, 4, 8}) to jointly
    cover O(L · log L) positions at O(L · log L) cost — without depending on
    ViennaRNA, so pairs ViennaRNA misses still have an information path.

    Ring (circular) indexing is mandatory for circRNA: linear i + k·r would
    fall off the end near the sequence boundary, exactly where the BSJ
    closure constraint lives. We take (i + k·r) % L so the dilated pattern
    wraps across the head-tail junction.

    The mask is built per-batch (not shared across the batch), because
    variable-length padding means valid positions differ per sample.

    Args:
        d_model: Hidden dimension
        dilation_rates: tuple of ints, one dilation rate per head.
            Empty -> module is a no-op (returns input unchanged).
        dropout: Dropout rate on attention weights (default: 0.1)
    """

    def __init__(self, d_model: int, dilation_rates: Tuple[int, ...] = (),
                 dropout: float = 0.1):
        super().__init__()
        self.d_model = d_model
        self.dilation_rates = tuple(dilation_rates)
        self.n_heads = len(self.dilation_rates)

        if self.n_heads == 0:
            return  # no-op; forward returns x unchanged

        assert d_model % self.n_heads == 0, (
            f"d_model ({d_model}) must be divisible by n_dilated_heads "
            f"({self.n_heads})"
        )
    ...
    def _build_head_key_index(self, L: int, r: int, device) -> torch.Tensor:
        """Build (L, L//r) key-position index for one dilated head.

        key_idx[i, k] = (i + k * r) % L  for k in 0..(L/r - 1).
        Head attends query i to keys on its dilated ring subsample only,
        so scores stay (B, L, L/r) — no L*L materialization. Memory drops
        from O(B·H·L²) (dense-then-mask) to O(B·L²/r_h) per head.

        k ranges over a full period L/r, so every reachable key (including
        i itself at k=0) appears exactly once — no duplication; self at k=0
        is standard in dilated attention.
        """
        n_keys = max(L // r, 1)
        arange = torch.arange(L, device=device, dtype=torch.long)
        k_steps = torch.arange(n_keys, device=device, dtype=torch.long)
        return (arange.unsqueeze(1) + k_steps.unsqueeze(0) * r) % L  # (L, n_keys)
    ...
    def forward(self, x: torch.Tensor,
        ...
        Args:
            x: (B, L, D)
            key_padding_mask: (B, L) bool, True = position is padding (ignored)

        Returns:
            (B, L, D)

        Adaptive per-head routing:
          - Small dilation (n_keys >= L/2, i.e. r in {1,2}): gather would
            materialize a (B, L, L/2, dh) tensor ~ the size of dense scores,
            so the gather is counterproductive. Route to dense matmul with a
            ring mask — single-head dense is (B,1,L,L), already 1/H of the
            full dense-then-mask cost, and cheaper than gather+v materialization.
          - Large dilation (r >= 4, n_keys <= L/4): gather only the L/r keys
            per query. Peak memory O(B·L²/r_h) per head, no L*L materialization
            beyond the dense small-r heads.
        """
        if self.n_heads == 0:
            return x
        ...
        out = torch.empty(B, H, L, dh, device=x.device, dtype=x.dtype)
        # Threshold: gather pays off only when the gathered tensor is much
        # smaller than dense scores. n_keys < L/2 (r >= 4 for typical L).
        GATHER_THRESHOLD = L // 2

        for h in range(H):
            r = int(self.dilation_rates[h])
            key_idx = self._build_head_key_index(L, r, device)  # (L, n_keys)
            ...
            if n_keys >= GATHER_THRESHOLD and n_keys < L:
                # --- dense path: small dilation, gather not worth it ---
                # single-head dense scores (B, 1, L, L), then ring-mask.
                scores = torch.matmul(qh.unsqueeze(1), kh.unsqueeze(1).transpose(-1, -2))  # (B,1,L,L)
                scores = scores * self.scale
                arange = torch.arange(L, device=device, dtype=torch.long)
                delta = (arange.view(1, L) - arange.view(L, 1)) % L  # (L, L)
                ring_mask = (delta % r) != 0  # True = not reachable on this ring
                scores = scores.masked_fill(ring_mask.view(1, 1, L, L), float('-inf'))
                if key_padding_mask is not None:
                    kpm = key_padding_mask[:, None, None, :]  # (B,1,1,L)
                    scores = scores.masked_fill(kpm, float('-inf'))
                attn = torch.softmax(scores, dim=-1)  # (B,1,L,L)
                attn = torch.nan_to_num(attn, nan=0.0)
                attn = self.dropout(attn)
                oh = torch.matmul(attn, vh.unsqueeze(1)).squeeze(1)  # (B, L, dh)
            else:
                # --- sparse gather path: large dilation ---
                kidx_b = key_idx.unsqueeze(0).expand(B, L, n_keys)  # (B, L, n_keys)
                ba = torch.arange(B, device=device).view(B, 1, 1)
                kg = kh[ba, kidx_b]  # (B, L, n_keys, dh)
                vg = vh[ba, kidx_b]  # (B, L, n_keys, dh)
                scores = torch.einsum('bld,blnd->bln', qh, kg) * self.scale  # (B, L, n_keys)
                if key_padding_mask is not None:
                    pad_gathered = key_padding_mask[ba, kidx_b]  # (B, L, n_keys)
                    scores = scores.masked_fill(pad_gathered, float('-inf'))
                attn = torch.softmax(scores, dim=-1)  # (B, L, n_keys)
                attn = torch.nan_to_num(attn, nan=1.0 / n_keys)
                attn = self.dropout(attn)
                oh = torch.einsum('bln,blnd->bld', attn, vg)  # (B, L, dh)
...
class SparsePairAttention(nn.Module):
    ...
    Two parallel attention branches, fused:
      1. ViennaRNA branch — attends only to Top-K candidate pairs (chemistry
         prior). Implemented via nn.MultiheadAttention with a per-batch
         candidate mask.
      2. Dilated ring branch (optional) — DilatedRingAttention with multiple
         dilation rates (position prior). Independent of ViennaRNA, so pairs
         the chemistry prior misses still get an information path.

    When dilation_rates is empty, branch 2 is a no-op and behavior is
    identical to the original ViennaRNA-only module (backward compatible).

    Args:
        d_model: Hidden dimension
        n_heads: Number of ViennaRNA-branch attention heads (default: 4)
        dropout: Dropout rate (default: 0.1)
        dilation_rates: tuple of per-head dilation rates for the dilated
            branch. Empty (default) disables the branch.
    """
    ...
    def __init__(self, d_model: int, n_heads: int = 4, dropout: float = 0.1,
        ...
        # Dilated ring branch (no-op if dilation_rates is empty)
        self.dilated = DilatedRingAttention(
            d_model, dilation_rates=self.dilation_rates, dropout=dropout
        )
        # Fuse the two branches. If dilated is a no-op this stays a clean
        # residual passthrough on the ViennaRNA output.
        self.branch_fuse = nn.Linear(d_model, d_model) if self.dilation_rates else None
    ...
    def forward(
        ...
        Args:
            ...
            key_padding_mask: (B, L) bool, True = padding position (optional)
        ...
        # ViennaRNA branch.
        # nn.MultiheadAttention's 3D attn_mask is (num_heads, L, L), NOT
        # (B, L, L) — it cannot express a per-sample sparse pattern. The
        # candidate mask is symmetric and, within a padded batch, dominated
        # by the shared ViennaRNA prior for valid positions, so we use the
        # first sample's mask as the head-shared sparse pattern and let
        # key_padding_mask handle per-sample padding differences.
        attn_mask = torch.zeros(L, L, device=x.device, dtype=x.dtype)
        attn_mask = attn_mask.masked_fill(~candidate_mask[0], float('-inf'))
        ...
        # Guard: a query row with zero candidates (all -inf in attn_mask)
        # yields NaN from softmax inside MHA. Sanitize to 0 contribution.
        attn_out = torch.nan_to_num(attn_out, nan=0.0, posinf=0.0, neginf=0.0)

        # Dilated ring branch (position prior, ViennaRNA-independent)
        if self.branch_fuse is not None:
            dil_out = self.dilated(x_norm, key_padding_mask=key_padding_mask)
            attn_out = self.branch_fuse(attn_out + dil_out)
...
class Scheme8Config:
    ...
    # Dilated ring attention branch — position-prior counterpart to the
    # ViennaRNA chemistry-prior candidate mask. Empty tuple (default) disables
    # the branch and preserves original ViennaRNA-only behavior.
    # Rates {2, 4, 8} cover O(L·log L) positions per layer at sub-O(L²) cost.
    dilation_rates: Tuple[int, ...] = ()
...
class Scheme8Model(nn.Module):
    ...
    def __init__(self, config: Optional[Scheme8Config] = None):
        ...
        # TrunkOutput adapter: synthesizes dense pair_repr from token + pair_probs
        # (S8 internally uses SPARSE pair — we don't densify to preserve O(L*K)
        # memory property; the synthesized pair_repr is honest about being
        # learned-prior rather than trunk-refined).
        from .trunk_output import TrunkOutputAdapter, PairPriorSynthesizer
        self.trunk_adapter = TrunkOutputAdapter(
            d_model=self.config.d_model,
            c_z=self.config.d_pair,
            structure_method="scheme8_sparse_pair",
        )

        # Optional immune-fingerprint head hook (scheme choice (c)).
        # Attach externally via attach_immune_head(head).
        self.immune_head: Optional[nn.Module] = None

    def attach_immune_head(self, head: nn.Module) -> None:
        """Attach a pluggable immune-fingerprint head (e.g. shared
        ImmuneFingerprintHeads). The head is called at the end of forward
        with (sequence_repr, pair_repr, torus_coords=None, pair_probs)."""
        self.immune_head = head

    def _encode_single_for_trunk(
        self,
        seq_tokens: torch.Tensor,
        pair_probs: Optional[torch.Tensor],
    ) -> torch.Tensor:
        """Run Mamba + sparse pair pipeline ONCE at the end of forward to
        produce the per-residue representation (single_repr) for TrunkOutput.

        This is a cheap (~10% extra time) re-encoding; we keep the diffusion
        core untouched to avoid gradient complications. The trunk_adapter
        then synthesizes a dense pair_repr from single_repr + pair_probs.
        """
        cond = self._encode_sequence(seq_tokens)
        cond = self._sparse_pair_pipeline(
            cond, seq_tokens, pair_probs, key_padding_mask=None,
        )
        return cond  # (B, L, d_model)
    ...
    def _build_pair_prior(
        ...
        Returns:
            ...
            key_padding_mask: (B, L) bool, True = padding position
                (used to keep ViennaRNA/dilated attention from attending
                padded keys in variable-length batches)
        """
        ...
        # key_padding_mask[b, i] = True iff i >= lengths[b]
        arange_L = torch.arange(L, device=device).unsqueeze(0)  # (1, L)
        key_padding_mask = arange_L >= lengths.unsqueeze(1)      # (B, L)

        return pair_probs_batch, candidate_mask_batch, key_padding_mask
    ...
    def _sample_with_mask(
        ...
        # TrunkOutput 协议 + 免疫头（可选）
        single_for_trunk = self._encode_single_for_trunk(seq_tokens, pair_probs)
        trunk_out = {
            "coords": coords,
            "sequence_repr": single_for_trunk,
            "pair_probs": pair_probs if pair_probs is not None else None,
            "structure_method": "scheme8_sparse_pair",
        }
        adapted = self.trunk_adapter(trunk_out)

        # Optional immune-fingerprint head.
        # S8's diffusion loop outputs Cartesian coords → pass as
        # cartesian_coords so the heads use the 3D kNN-density exposure proxy.
        if self.immune_head is not None:
            adapted["immune"] = self.immune_head(
                sequence_repr=adapted["sequence_repr"],
                pair_repr=adapted["pair_repr"],
                torus_coords=None,  # S8 无 torus 参数
                pair_probs=pair_probs,
                cartesian_coords=coords,  # (B, L, 3) diffusion 采样的真实 3D 结构
            )
    ...
    def iterative_refinement(
        ...
        pair_probs, candidate_mask, key_padding_mask = self._build_pair_prior(seq_tokens)
        ...
        # output 已是 _sample_with_mask 返回的 TrunkOutput-compliant dict
        # （含 coords/sequence_repr/pair_repr/pair_probs/closure_dist/immune 等全部字段）。
        # 这里只补 recycle 专属信息，不丢协议字段。
        output['candidate_mask'] = candidate_mask
        output['rmse_history'] = getattr(self, '_last_rmse_history', [])
        # override method 标记为 recycle 版本
        output['structure_method'] = 'scheme8_sparse_pair_recycle'
        return output
    ...
    def _sparse_pair_pipeline(
        ...
        Args:
            ...
            key_padding_mask: (B, L) bool, True = padding (from _build_pair_prior
                or external). Forwarded to SparsePairAttention so the ViennaRNA
                branch (key_padding_mask arg of nn.MultiheadAttention) and the
                dilated branch (gathered-key masking) both ignore padded keys.
        ...
        if pair_probs is None:
            pair_probs, candidate_mask, kpm = self._build_pair_prior(seq_tokens)
            if key_padding_mask is None:
                key_padding_mask = kpm
        else:
            # Use provided pair_probs, compute Top-K mask
            candidate_mask = self.pair_selector(pair_probs)
            # Derive padding mask from seq_tokens if caller didn't supply one.
            # token id 4 == N/padding per _build_pair_prior's convention.
            if key_padding_mask is None:
                key_padding_mask = (seq_tokens == 4)
        ...
        for sparse_layer, gate_layer in zip(self.sparse_pair_attn, self.global_gate):
            x = sparse_layer(x, candidate_mask, key_padding_mask=key_padding_mask)
            x = gate_layer(x)
    ...
    def _denoise(
        ...
        cond = self._encode_sequence(seq_tokens)
        cond = self._sparse_pair_pipeline(
            cond, seq_tokens, pair_probs, key_padding_mask=key_padding_mask
        )
    ...
    def _train_step(
        ...
        # Padding mask: token id 4 == N/padding (matches _build_pair_prior).
        # Cheap to compute once per step; avoids re-deriving per diffusion
        # call inside _sparse_pair_pipeline's fallback.
        key_padding_mask = (seq_tokens == 4)
