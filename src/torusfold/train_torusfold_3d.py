...
class CircRNA3DModel(nn.Module):
    ...
    def __init__(self, d_hidden=128, n_layers=4):
        ...
        # S1 (CircRNA3DModel, EGNN) has no pair_probs input and no torus
        # parameterization; synthesizer builds pair_repr from node feat h
        # (uniform pair prior fallback).
        self.trunk_adapter = TrunkOutputAdapter(
            d_model=d_hidden,
            c_z=d_hidden,
            structure_method="circrna_3d_egnn",
        )
    ...
    def forward(self, seq_ids: torch.Tensor, lengths=None) -> dict:
        ...
        # S1 返回 h 作为 sequence_repr (B, L, d_hidden)，pair_probs=None
        #（synthesizer 用 uniform fallback），pair_repr 由 synthesizer 合成。
        trunk_out = {
            "coords": coords,
            "sequence_repr": h,
            "pair_probs": None,  # S1 有 no explicit pair_probs input
            "structure_method": "circrna_3d_egnn",
        }
        adapted = self.trunk_adapter(trunk_out)

        # Optional immune-fingerprint head.
        # S1 outputs Cartesian coords → pass them as cartesian_coords so the
        # DRACH m6A head uses the 3D kNN-density exposure proxy (path 2),
        # not the 2D pair_probs fallback.
        if self.immune_head is not None:
            adapted["immune"] = self.immune_head(
                ...
                torus_coords=None,  # S1 无 torus 参数
                pair_probs=None,    # S1 无显式 pair_probs input
